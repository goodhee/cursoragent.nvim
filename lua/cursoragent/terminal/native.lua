---Native Neovim terminal provider for Cursor Agent.
---@module 'cursoragent.terminal.native'

local M = {}

local logger = require("cursoragent.logger")
local utils = require("cursoragent.util")
...
local config = require("cursoragent.terminal").defaults
...
local function find_existing_cursor_agent_terminal()
  local buffers = vim.api.nvim_list_bufs()
  for _, buf in ipairs(buffers) do
    if vim.api.nvim_buf_is_valid(buf) and vim.api.nvim_buf_get_option(buf, "buftype") == "terminal" then
      -- Check if this is a Cursor Agent terminal by examining the buffer name or terminal job
      local buf_name = vim.api.nvim_buf_get_name(buf)
      -- Terminal buffers often have names like "term://..." that include the command
      if buf_name:match("cursor%-agent") or buf_name:match("cursoragent") then
        -- Additional check: see if there's a window displaying this buffer
        local windows = vim.api.nvim_list_wins()
        for _, win in ipairs(windows) do
          if vim.api.nvim_win_get_buf(win) == buf then
            logger.debug("terminal", "Found existing Cursor Agent terminal in buffer", buf, "window", win)
            return buf, win
          end
        end
      end
...
---@param term_config CursorAgentTerminalConfig
...
function M.open(cmd_string, env_table, effective_config, focus)
  ...
  if is_valid() then
    ...
    -- Check if there's an existing Cursor Agent terminal we lost track of
    local existing_buf, existing_win = find_existing_cursor_agent_terminal()
    if existing_buf and existing_win then
      ...
      logger.debug("terminal", "Recovered existing Cursor Agent terminal")
      if focus then
      ...
      if not open_terminal(cmd_string, env_table, effective_config, focus) then
        vim.notify("Failed to open Cursor Agent terminal using native fallback.", vim.log.levels.ERROR)
      end
    end
  end
end
...
---@param cmd_string string
---@param env_table table
---@param effective_config CursorAgentTerminalConfig
    ...
    if has_buffer then
      ...
      if existing_buf and existing_win then
        ...
        if not open_terminal(cmd_string, env_table, effective_config) then
          vim.notify("Failed to open Cursor Agent terminal using native fallback (simple_toggle).", vim.log.levels.ERROR)
        end
      end
    end
  end
...
function M.focus_toggle(cmd_string, env_table, effective_config)
  ...
  if has_buffer then
    ...
    if existing_buf and existing_win then
      ...
      -- No existing terminal found, create a new one
      if not open_terminal(cmd_string, env_table, effective_config) then
        vim.notify("Failed to open Cursor Agent terminal using native fallback (focus_toggle).", vim.log.levels.ERROR)
      end
    end
  end
end
...
--- @type CursorAgentTerminalProvider
return M
