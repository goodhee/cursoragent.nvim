---@brief WebSocket server for Cursor Agent Neovim integration
local cursoragent_main = require("cursoragent") -- Added for version access
local logger = require("cursoragent.logger")
local tcp_server = require("cursoragent.server.tcp")
local tools = require("cursoragent.tools.init") -- Added: Require the tools module
...
---@param config CursorAgentConfig Configuration options
...
function M.start(config, auth_token)
  ...
  local callbacks = {
    ...
    on_connect = function(client)
      ...
      local main_module = require("cursoragent")
...
function M.stop()
  ...
  if _G.cursoragent_deferred_responses then
    _G.cursoragent_deferred_responses = {}
  end
...
function M._setup_deferred_response(deferred_info)
  ...
  if not _G.cursoragent_deferred_responses then
    _G.cursoragent_deferred_responses = {}
  end
  _G.cursoragent_deferred_responses[tostring(co)] = response_sender
  ...
  M.state.handlers = {
    ["initialize"] = function(client, params)
      return {
        ...
        serverInfo = {
          name = "cursoragent-neovim",
          version = cursoragent_main.version and cursoragent_main.version:string() or "0.1.0",
        },
      }
    end,
