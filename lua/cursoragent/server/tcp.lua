...
local client_manager = require("cursoragent.server.client")
local utils = require("cursoragent.server.utils")
...
---@param config CursorAgentConfig Server configuration
...
function M.start_ping_timer(server, interval)
  ...
  timer:start(interval, interval, function()
    ...
    if is_wake_from_sleep then
      ...
      require("cursoragent.logger").debug(
        "server",
        string.format(
    ...
    for _, client in pairs(server.clients) do
      if client.state == "connected" then
        ...
        if client_manager.is_client_alive(client, interval * 2) then
          ...
          require("cursoragent.logger").info(
            "server",
