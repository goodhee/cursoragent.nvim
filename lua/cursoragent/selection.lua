---Manages selection tracking and communication with the Cursor Agent server.
---@module 'cursoragent.selection'
local M = {}
...
function M._clear_autocommands()
  vim.api.nvim_clear_autocmds({ group = "CursorAgentSelection" })
end
...
function M.update_selection()
  ...
  -- If the buffer name starts with "✻ [Cursor Agent]", do not update selection
  if buf_name and string.sub(buf_name, 1, string.len("✻ [Cursor Agent]")) == "✻ [Cursor Agent]" then
  ...
  -- If the current buffer is the Cursor Agent terminal, do not update selection
  if terminal then
    local cursoragent_term_bufnr = terminal.get_active_terminal_bufnr()
    if cursoragent_term_bufnr and current_buf == cursoragent_term_bufnr then
      -- Cancel any pending demotion if we switch to the Cursor Agent terminal
      if M.state.demotion_timer then
        M.state.demotion_timer:stop()
        M.state.demotion_timer:close()
        M.state.demotion_timer = nil
...
function M.handle_selection_demotion(original_bufnr_when_scheduled)
  ...
  -- Condition 1: Switched to Cursor Agent Terminal
  if cursoragent_term_bufnr and current_buf == cursoragent_term_bufnr then
  ...
  -- Condition 3: Still in Original Buffer & Not Visual & Not Cursor Agent Term -> Demote
...
---Sends the selection update to the Cursor Agent server.
...
---Sends the current selection to Cursor Agent.
...
function M.send_current_selection()
  if not M.state.tracking_enabled or not M.server then
    logger.error("selection", "Cursor Agent is not running")
    return
  end
  ...
  vim.api.nvim_echo({ { "Selection sent to Cursor Agent", "Normal" } }, false, {})
end
...
function M.send_at_mention_for_visual_selection(line1, line2)
  ...
  -- Check if Cursor Agent integration is running (server may or may not have clients)
  local cursoragent_main = require("cursoragent")
  if not cursoragent_main.state.server then
    logger.error("selection", "Cursor Agent integration is not running.")
    return false
  end
  ...
  local success, error_msg = cursoragent_main.send_at_mention(file_path, start_line, end_line, "CursorAgentSend")
